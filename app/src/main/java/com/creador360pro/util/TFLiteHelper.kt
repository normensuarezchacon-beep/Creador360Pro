...
object TFLiteHelper {
    ...
    fun initialize(context: Context) {
        ...
        // Cargar DeepLabV3 (solo si está disponible)
        try {
    ...
    private fun detectDeviceTier(context: Context): DeviceTier {
        ...
        score += when { totalRamGB <= 3.0 -> 0; totalRamGB <= 6.0 -> 1; else -> 2 }
        score += when { Build.VERSION.SDK_INT < Build.VERSION_CODES.Q -> 0; Build.VERSION.SDK_INT < Build.VERSION_CODES.UPSIDE_DOWN_CAKE -> 1; else -> 2 }
        val cpuInfo = getCPUInfo()
        score += when {
            cpuInfo.contains("Helio G2") || cpuInfo.contains("Helio P2") || cpuInfo.contains("Snapdragon 4") || cpuInfo.contains("Unisoc") || cpuInfo.contains("Spreadtrum") -> 0
            cpuInfo.contains("Helio G3") || cpuInfo.contains("Helio G4") || cpuInfo.contains("Helio G8") || cpuInfo.contains("Helio P6") || cpuInfo.contains("Snapdragon 6") -> 1
            cpuInfo.contains("Helio G9") || cpuInfo.contains("Dimensity") || cpuInfo.contains("Snapdragon 7") || cpuInfo.contains("Snapdragon 8") || cpuInfo.contains("Exynos 9") || cpuInfo.contains("Exynos 2") -> 2
            else -> 1
        }
        val gpuInfo = getGPUInfo()
        score += when {
            gpuInfo.contains("Mali-G31") || gpuInfo.contains("Mali-G52") || gpuInfo.contains("PowerVR") || gpuInfo.contains("Adreno 3") || gpuInfo.contains("Adreno 5") || gpuInfo.contains("Mali-400") -> 0
            gpuInfo.contains("Mali-G57") || gpuInfo.contains("Mali-G68") || gpuInfo.contains("Mali-G72") || gpuInfo.contains("Mali-G76") || gpuInfo.contains("Adreno 6") -> 1
            gpuInfo.contains("Mali-G77") || gpuInfo.contains("Mali-G78") || gpuInfo.contains("Mali-G710") || gpuInfo.contains("Adreno 7") || gpuInfo.contains("Immortalis") -> 2
            else -> 1
        }
        return when { score <= 2 -> DeviceTier.LOW; score <= 5 -> DeviceTier.MEDIUM; else -> DeviceTier.HIGH }
    }
    ...
    private fun getCPUInfo(): String {
        return try {
            ...
            while (reader.readLine().also { line = it } != null) {
                if (line!!.contains("Hardware") || line!!.contains("model name")) cpuInfo.append(line).append(" ")
            }
            reader.close()
            cpuInfo.toString().ifEmpty { Build.HARDWARE ?: "Desconocido" }
        } catch (e: Exception) { Build.HARDWARE ?: "Desconocido" }
    }
    ...
    private fun getGPUInfo(): String {
        return try {
            ...
            var line: String?
            while (reader.readLine().also { line = it } != null) {
                if (line!!.contains("GLES") || line!!.contains("GL_")) gpuInfo.append(line).append(" ")
            }
            reader.close()
            gpuInfo.toString().ifEmpty { "GPU no detectada" }
        } catch (e: Exception) { "GPU no disponible" }
    }

    fun isAvailable(): Boolean = mediapipeLoaded || deeplabLoaded

    fun getActiveModelName(): String {
        return when { deeplabLoaded -> "DeepLabV3 (alta precisión)"; mediapipeLoaded -> "MediaPipe (optimizado)"; else -> "Ninguno" }
    }
    ...
    fun getDeviceInfo(context: Context): String {
        ...
        val tierName = when (deviceTier) { DeviceTier.LOW -> "Baja"; DeviceTier.MEDIUM -> "Media"; DeviceTier.HIGH -> "Alta" }
    ...
    fun getRequiredSpecs(): String {
        return "• 4 GB de RAM o más\n• Android 10 o superior\n• Cualquier procesador con soporte TFLite"
    }
    ...
    private fun removeBackgroundMediapipe(originalBitmap: Bitmap): Bitmap? {
        ...
        val outputBuffer = ByteBuffer.allocateDirect(4 * inputSize * inputSize * 1).apply { order(ByteOrder.nativeOrder()) }
    ...
    private fun removeBackgroundDeeplab(originalBitmap: Bitmap): Bitmap? {
        ...
        // Obtener forma del tensor de salida
        val outputTensor = interpreter.getOutputTensor(0)
        val outputShape = outputTensor.shape()
        val numClasses = outputShape[outputShape.size - 1].toInt()

        val outputBuffer = ByteBuffer.allocateDirect(4 * inputSize * inputSize * numClasses).apply { order(ByteOrder.nativeOrder()) }
        interpreter.run(inputBuffer, outputBuffer)
        outputBuffer.rewind()

        // Construir máscara: el índice 15 suele ser "person" en DeepLabV3 (Pascal VOC)
        val personClassIndex = 15
        val maskBitmap = Bitmap.createBitmap(inputSize, inputSize, Bitmap.Config.ARGB_8888)
        val maskPixels = IntArray(inputSize * inputSize)
        ...
        for (i in maskPixels.indices) {
            ...
            maskPixels[i] = if (maxClass == personClassIndex) Color.WHITE else Color.BLACK
        }
